...
| Entscheidung & Begrundung | v1 nutzt zwei Expositionsprofile: (1) Default `localhost-only`; (2) optional `LAN-only` als expliziter Opt-in. WAN/Internet-Exposition und VPN/Overlay-Zugriff sind in v1 nicht unterstuetzt. LAN-only ist nur zulaessig, wenn Mindestschutz fuer Nicht-Loopback aktiv ist (Token-basierte Authentisierung) und Netzgrenzen auf Docker/Host/Firewall-Schicht gesetzt sind. Mit Umsetzung von Vikunja `P1-004` ist LAN-only nur bei expliziter Non-Loopback-Publish-Konfiguration und gesetztem `RAG_API_TOKEN` zulaessig. Diese Entscheidung erhaelt sichere Defaults, erlaubt den gewuenschten LAN-Betrieb kontrolliert und verhindert implizite Oeffnung nach aussen. |
...
- LAN-Betrieb ist ein bewusster Opt-in und erfordert zusaetzliche Betriebsaufgaben (Token, Netzgrenze, Dokumentation, Neustart bei Token-Rotation).
...
- `LAN-only`: Non-Loopback-Publish ohne `RAG_API_TOKEN` ist ungueltig; `/mcp` und `/mcp/` weisen Requests ohne gueltiges Bearer-Token ab.
