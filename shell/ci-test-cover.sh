#!/bin/sh
set -eu

: "${COVERAGE_MIN:=60}"

sh ./shell/dockerfile-test.sh test -count=1 -covermode=atomic -coverprofile=coverage.out ./...
sh ./shell/dockerfile-test.sh tool cover -func=coverage.out > coverage.txt
awk -v min="$COVERAGE_MIN" '/^total:/ { gsub(/%/, "", $3); if (($3 + 0) < (min + 0)) { printf("coverage %.1f%% is below minimum %.1f%%\n", $3, min); exit 1 }; found=1 } END { if (!found) { print "coverage total not found"; exit 1 } }' coverage.txt
